...
Unofficial client for Telegram on legacy iOS.

This is an unofficial legacy iOS Telegram client fork. It does not include private Telegram API credentials.

Before building, provide your own Telegram API credentials in the project configuration/macros:

```objc
#define SETUP_API_ID(apiId) apiId = <your api id>;
#define SETUP_API_HASH(apiHash) apiHash = @"<your api hash>";
```

Do not commit signing files, provisioning profiles, packaged IPAs, local build products, or Xcode user data.
